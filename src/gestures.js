...
/* ЗАХВАТ: морозим точку отсчёта по ТРЁМ осям и стартовые значения ВСЕХ параметров слота.
   ⛔ base — ПОЛНЫЙ массив ПО ИНДЕКСУ ПАРАМЕТРА, и фильтровать его НЕЛЬЗЯ. Соблазн «не захватывать
   фиксированные/чужие» ломает соответствие индексов: base[i] перестал бы отвечать params[i], и палец
   поехал бы НЕ ТОТ параметр — на слух это читается как «оси перепутались». Фильтр по режиму/руке живёт
   РОВНО В ОДНОМ месте — в пофреймовом цикле записи (ниже), и больше нигде.
    ...
    if(S.pinch){
      ...
      if(S.zone==='fx'){
        ...
        if(A){
          ...
          for(let i=0;i<np;i++){
            const pa=sl.params[i];
            /* ФИЛЬТР ПО РЕЖИМУ И РУКЕ — ЕДИНСТВЕННОЕ, что слайс 2.6.1 добавил в этот цикл.
               ...
               иначе две руки подерутся за одну величину. Именно этот фильтр и ЕСТЬ механизм разделения
               одного эффекта между руками; в 2.6.1 он тривиально истинен на всей раскладке, поэтому
               поведение обязано быть НЕОТЛИЧИМЫМ. Всё ниже — прежняя арифметика, символ в символ. */
