...
export const DICTS = {
  en: {
    ...
    'tutor.spOrient.prompt':'Two roles at once needs a wide, landscape screen. Turn your phone sideways (or widen the window on a computer) — the lesson continues on its own once it\'s wide.',
    'tutor.spOrient.detail':'Split-screen puts two instruments side by side, which needs a wide screen. Rotate your device to landscape, or widen the window on a desktop, and this step moves on by itself.',
    'tutor.spOn.prompt':'Tap the ◨ button at the top. The screen splits into two halves.',
    'tutor.spOn.detail':'The ◨ button (top bar, landscape only) turns split-screen on. Tap it — the screen divides into a left half and a right half, each its own instrument.',
    'tutor.spHalves.prompt':'Each half is a different instrument, and the half you pinch in decides which you play — not which hand. Pinch in the Solo half for a melody note, then the Chords half for a chord — together that\'s a song.',
    'tutor.spHalves.detail':'Pinch a finger in the Solo half, then in the Chords half — a melody line in one, full chords in the other. Use one hand or both; here it\'s the place that chooses, not the hand.',
    'tutor.spRoles.prompt':'You choose what each half plays. Each half has its own role button at the top — tap one to switch that half to another instrument, say Bass or Drums. It skips the role the other half already holds, so the two can never be the same.',
    'tutor.spRoles.detail':'The two role buttons at the top (one per half) cycle Solo → Chords → Bass → Drums, each skipping the role the other half holds — so the halves are always different. Change a half and hear it; you can set them back to Solo and Chords whenever you like.',
    'tutor.spFinal.prompt':'Now each half is a full instrument — and Hand functions apply per half by that half\'s role, so a solo half keeps its effects and theremin. Rotate back to portrait and split turns off by itself; nothing breaks.',
    ...
    'lesson.split.desc':'Split the screen — each half is its own instrument, either hand.',
  },
  ru: {
    ...
    // обучение — урок «Две роли сразу» (сплит)
    'tutor.spOrient.prompt':'«Две роли сразу» нужен широкий, ландшафтный экран. Поверните телефон набок (или расширьте окно на компьютере) — урок продолжится сам, как только станет широко.',
    'tutor.spOrient.detail':'Сплит-экран ставит два инструмента рядом — нужен широкий экран. Поверните устройство в ландшафт или расширьте окно на компьютере, и этот шаг пройдёт сам.',
    'tutor.spOn.prompt':'Нажмите кнопку ◨ вверху. Экран разделится на две половины.',
    'tutor.spOn.detail':'Кнопка ◨ (верхний бар, только в ландшафте) включает сплит-экран. Нажмите — экран делится на левую и правую половину, каждая — свой инструмент.',
    'tutor.spHalves.prompt':'Каждая половина — свой инструмент, и играет та, в чьей половине щипок, — а не какая рука. Щипните в соло-половине — нота мелодии, затем в аккордовой — аккорд; вместе это песня.',
    'tutor.spHalves.detail':'Щипните пальцем в соло-половине, потом в аккордовой — в одной мелодия, в другой полные аккорды. Одной рукой или двумя; здесь выбирает МЕСТО, а не рука.',
    'tutor.spRoles.prompt':'Вы выбираете, что играет каждая половина. У каждой своя кнопка роли вверху — нажмите, чтобы сменить половину на другой инструмент, например Бас или Ударные. Она пропускает роль другой половины, поэтому две половины никогда не совпадут.',
    'tutor.spRoles.detail':'Две кнопки ролей вверху (по одной на половину) листают Соло → Аккорды → Бас → Ударные, пропуская роль другой половины — так половины всегда разные. Смените половину и послушайте; вернуть Соло и Аккорды можно в любой момент.',
    'tutor.spFinal.prompt':'Теперь каждая половина — полноценный инструмент, а «Функции рук» применяются на каждую половину по её роли, так что у соло-половины остаются её эффекты и терменвокс. Поверните обратно в портрет — сплит выключится сам, ничего не сломается.',
    ...
    'lesson.split.desc':'Разделите экран — каждая половина свой инструмент, любой рукой.',
  },
  es: {}, de: {},
};
