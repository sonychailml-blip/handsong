...
/* Пометить область и включить/выключить повтор. ⛳ ЕДИНСТВЕННЫЙ ПИСАТЕЛЬ rgn — как laneNew у дорожек.
   ⚠️ ГРАНИЦЫ СТАВИТ ВЫЗЫВАЮЩИЙ, и сегодня вызывающий ровно один: кнопка ⟳ помечает ВСЮ ПЕСНЮ
   ([0, songLen]). Подобласть («от такта до такта») требует ТАЙМЛАЙНА, по которому её видно и можно
   тащить, — а его в этом слайсе нет, и рисовать половину интерактива хуже, чем честно подождать
   редактора. Механизм при этом уже пролётный: придёт таймлайн — поменяется только КТО зовёт setRegion. */
function setRegion(from,to,on){
  const R=loop.rgn;
  R.from=Math.max(0,from); R.to=Math.max(R.from,to); R.on=!!on;
  schedInvalidate();          // временнáя карта сменилась — курсоры и планировщик обязаны перепривязаться
}
...
function push(fn,a,at){
  ...
  const e= at!=null ? at : songAt((AC.currentTime-loop.t0)*loop.bpm/60);
...
function scheduleLayers(){
  ...
  while(loop.sched<horizon-1e-9 && guard++<64){
    ...
    if(cyc && segEnd>=edge-1e-9) releaseLoopLayersAt(loop.t0+edge*spb);
    loop.sched=segEnd;
  }
}
...
function tick(){
  ...
  if(m<0)return;                                      // отсчёт: только щелчки/планирование
  const p=songAt(m);                                  // ПЕСЕННАЯ доля (при выключенной области — она же)
  if(p<loop.pos){                                     // заворот области: позиция ушла назад — тот же тест, что был у петли
...
function clearRec(){
  clearPump(); events.length=0; setRegion(0,0,false); schedInvalidate(); laneReset(); loop.on=false; setRecording(false); softAllOff(); droneOff();   // ✕ — песни больше нет, значит нет ни дорожек, ни области повтора: помеченный участок пустой песни был бы ложью на экране
  hooks.loop && hooks.loop(false);
}
...
export {
  ...
  setRegion, regionOn, cycling,   // S3.4: пометить область повтора / включена ли она (ui — кнопка ⟳; draw читает сами границы из loop.rgn)
