...
let vBase=0;                 // первый номер v, свободный от УЖЕ ЛЕЖАЩИХ в целевом слое нот (S3.5a) — ставит onRec на старте взятого, см. layerSlotTop
...
function push(fn,a){
  ...
  const e=songAt((AC.currentTime-loop.t0)*loop.bpm/60);
...
/* ⛳ ВОССТАНОВЛЕНО В S3.4, И ИМЕННО КАК РЕШЕНИЕ, А НЕ КАК НАСЛЕДСТВО — тумбстоун S3.3 требовал
   ...
   ⚠️ ЧЕСТНАЯ ПЛАТА, ОСТАВЛЕННАЯ КАК БЫЛА: состояние записи здесь СБРАСЫВАЕТСЯ без записи выключений,
   поэтому нота, удержанная через границу, оставляет в дорожке leadOn без пары — её гасит на
   переигровке releaseLoopLayersAt, ровно как до S3.3. Писать здесь настоящий leadOff (как это делает
   recAllOff на явной остановке) — осмысленное УЛУЧШЕНИЕ, но это уже новое поведение, а не
   воскрешение; сознательно не в этом слайсе. */
