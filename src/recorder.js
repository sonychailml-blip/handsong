...
function recChOn(a){ if(!recording)return; recFoldRoll();
  const inst=a.inst??chIdx; push('chOn',{...a,inst});
  recCh={deg:a.deg,oct:a.oct,vol:a.vol,ty:a.ty,bri:a.bri,inst,sc:CUR(),sev:seventh}; }
function recChSet(a){
  if(!recording)return;
  recFoldRoll();
  let inst;
  if(!recCh){ inst=chIdx; push('chOn',{...a,inst}); }
  else if(a.deg!==recCh.deg||a.oct!==recCh.oct||a.ty!==recCh.ty||Math.abs(a.vol-recCh.vol)>REC_VOL_EPS||Math.abs((a.bri||0)-(recCh.bri||0))>REC_REV_EPS){ inst=recCh.inst; push('chSet',{...a}); }
  else return;
  recCh={deg:a.deg,oct:a.oct,vol:a.vol,ty:a.ty,bri:a.bri,inst,sc:CUR(),sev:seventh};
}
...
function onUndo(){
  ...
  if(recording && top!==curTake) recAllOff();
