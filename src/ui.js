...
function showScale(on){ panelScaleEl.classList.toggle('on',on); if(on)showLoop(false);
  /* ПЕРЕСБОРКА ДИНАМИЧЕСКИХ СЕКЦИЙ ИЗ ДАННЫХ ПРИ ОТКРЫТИИ — ровно та же причина, что у showLoop с
     ...
     renderFxCtl, — «одна точка входа на обе». Но это была та же связка ВЛАДЕНИЯ, которую 3.4.1 снял:
     конструктор больше не принадлежит «Функциям рук», и обе причины пересборки (реестр модулей
     заполняется в initAudio; браузер восстанавливает формы после отрисовки) относятся к нему НАПРЯМУЮ,
     а не по наследству от соседа. Разъехаться по-прежнему нечему: обе строчки стоят рядом. */
  if(on){ renderHandFn(); renderFxCtl(); }
