...
/* ФУНКЦИИ РУК: по выпадающему НА РУКУ (Левая/Правая) для КАЖДОЙ роли с записью (соло/бас/аккорды), что
   ...
   защёлка/удержание (ни 'fx', ни терменвокса). У ВСЕХ трёх ролей есть 'loop' (Лупер): рука нот не играет,
   а командует лупером щипками пальцев (см. gestures: fireLooperCmd). Смена — setHandFn + softAllOff. */
const HANDFN_OPTS={
  ld:[['fx','Эффекты'],['note','Ноты (непрерывно)'],['hold','Ноты (с удержанием)'],['therm','Терменвокс'],['loop','Лупер (управление)']],
