...
/* РАСКЛАДКА ЭФФЕКТОВ fx-руки — КОНСТРУКТОР ЭФФЕКТОВ, Пласт 2 (см. HANDOFF «КОНСТРУКТОР ЭФФЕКТОВ»).
   ...
   ⚠️ ДЕФОЛТ ВЫВЕДЕН ИЗ FX_META, а не переписан руками: «ведёт себя ровно как раньше» тогда верно ПО
   ПОСТРОЕНИЮ, а не потому, что кто-то аккуратно скопировал четыре строки. Отсюда требование к FX_META:
   её ПОРЯДОК обязан совпадать с FINGER_TIPS (там же и записано).
   Сегодня у каждого старого эффекта РОВНО ОДИН параметр на оси 'y' — это и есть нынешнее поведение;
   многопараметрические эффекты (реверб: длина+окраска) становятся достижимы в слайсе 2.3. */
export let fxLayout = FX_META.map(m=>({ fxId:m.k, params:[{axis:'y', inv:false}] }));
