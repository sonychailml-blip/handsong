...
/* СКОЛЬКО ПАЛЬЦЕВ РУКИ МОГУТ ЗВУЧАТЬ ОДНОВРЕМЕННО (1..4). К большому можно прижать несколько пальцев,
   ...
   никуда не делась, но переехала в ВЫБОР: если диагностика щипка (ниже) покажет, что безымянный/мизинец
   срабатывают ложно — потолок опускают тем же контролом. Ложная нота хуже несыгранной, но и потолок,
   молча режущий половину рук, — не решение. */
export let pinchFingers=4;
export const setPinchFingers=v=>{ pinchFingers=Math.max(1,Math.min(4,v|0)); };
/* ВРЕМЕННАЯ ДИАГНОСТИКА ЩИПКА: показать живые отношения dist(большой,палец)/размер ладони по каждому
   пальцу и пороги ON/OFF. Нужна, чтобы РЕШИТЬ по числам, сколько пальцев различаются надёжно, а не
   гадать. Удалить (вместе с drawPinchDebug и контролом в панели), когда решение принято. */
export let pinchDebug=false;
export const setPinchDebug=v=>{ pinchDebug=!!v; };
