...
/* ЦЕПИ ЭФФЕКТОВ ПО РОЛЯМ — КОНСТРУКТОР ЭФФЕКТОВ, Пласт 3.3 (см. HANDOFF «КОНСТРУКТОР ЭФФЕКТОВ»).
   ...
   ⚠️ ПУСТО — это [], а НЕ четыре пустых слота: пустой слот ({fxId:'',…}) — форма ПАЛЬЦЕВАЯ, а
   решения «цепь аккордов тоже индексируется пальцами» никто не принимал (это вопрос 3.4/3.5).
   ...
   ⚠️ ИНДЕКС В ЦЕПИ — ЭТО ВСЁ ЕЩЁ ПАЛЕЦ, а не позиция в цепи обработки, — хотя структура теперь
   зовётся цепью РОЛИ. Расхождение осознанное и временное: пальцевую индексацию распускает Пласт 3.4
   (меню ОТ ПАРАМЕТРА, где палец становится АДРЕСОМ у параметра, а не владельцем эффекта). До тех пор
   ⛔ НЕ ПЕРЕУПОРЯДОЧИВАТЬ массив, считая индекс местом в цепи: переставишь — переедут пальцы.
...
/* ⚠️ ФОРМА ПАРАМЕТРА (Пласт 2.6) — параметр больше НЕ обязан быть «пальцевым». Ровно два рода:
     { mode:'drive', hand:'fx'|'play', axis:'x'|'y'|'z', inv }   — ВЕДЁТСЯ смещением от точки захвата
...
export let fxChains = {
  ld:[
    { fxId:'reverb', params:[{mode:'drive',hand:'fx',axis:'y',inv:false},{mode:'drive',hand:'fx',axis:'x',inv:false},{mode:'drive',hand:'fx',axis:'z',inv:false}] },
    { fxId:'dly',    params:[{mode:'drive',hand:'fx',axis:'y',inv:false}] },
    { fxId:'drv',    params:[{mode:'drive',hand:'fx',axis:'y',inv:false}] },
    { fxId:'vib',    params:[{mode:'drive',hand:'fx',axis:'y',inv:false}] },
  ],
...
function hushUnassignedFx(){
  for(const m of FX_META)
    if(!fxChainOf('ld').some(sl=>sl&&sl.fxId===m.k)) fx[m.k]=FX_NEUTRAL;
}
...
/* ⚠️ ЭМИТИМ ПОЛНУЮ ФОРМУ (mode+hand), а не голые {axis,inv}. Параметр без mode жест-цикл ПРОПУСТИЛ бы
   по фильтру, и эффект молча перестал бы отзываться на палец. Пусть ошибка будет ГРОМКОЙ (её видно
   сразу), а не замаскированной оборонительным чтением вида (pa.mode||'drive'). */
/* ⚠️ СИГНАТУРЫ СЕТТЕРОВ БЕЗ РОЛИ, намеренно (Пласт 3.3): единственный вызывающий — renderFxCtl, а
   переключателя роли в меню ещё нет (это Пласт 3.4). Параметр с одним допустимым значением — мёртвая
   сложность. Тела ходят через fxChainOf, поэтому роль добавится ОДНИМ токеном, а не переписыванием.
   ⚠️ КОГДА ОНА ПРИДЁТ (3.4/3.5) — РОЛЬ ИДЁТ ПЕРВЫМ АРГУМЕНТОМ: setFxSlotId(role,slot,fxId,n), как у
   setHandFn(role,hand,fn) и setRectOctReg(role,v). И только ЯВНЫМ аргументом: выводить роль из
   глобального phoneInstr запрещено правилом CLAUDE.md (в сплите он не указывает на роль руки). */
export const setFxSlotId=(slot,fxId,nParams=1)=>{
  const CH=fxChainOf('ld');
  if(!CH[slot])return;
  const n=Math.max(1,nParams|0);
  CH[slot]={ fxId, params:Array.from({length:n},(_,i)=>(
    { mode:'drive', hand:'fx', axis:FX_AXIS_DEF[i%FX_AXIS_DEF.length], inv:false })) };
...
export const setFxParamAxis=(slot,pIdx,axis,inv,hand)=>{
  const s=fxChainOf('ld')[slot]; if(!s||!s.params[pIdx])return;
...
export const setFxParamMode=(slot,pIdx,mode)=>{
  const s=fxChainOf('ld')[slot]; if(!s||!s.params[pIdx])return;
  const p=s.params[pIdx];
...
export const setFxParamFixed=(slot,pIdx,v01)=>{
  const s=fxChainOf('ld')[slot]; if(!s||!s.params[pIdx])return;
  const v=+v01; s.params[pIdx].v01 = Math.max(0, Math.min(1, isNaN(v)?0:v));
};
