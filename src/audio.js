...
/* ================= МОДУЛЬ ЭФФЕКТА — ПЛАСТ 1: абстракция + ТОЛЬКО реверб =================
   ...
   • ПОДМЕС (mix) — свойство ПРИЦЕПКИ, а не эффекта: у реверба прицепок ДВЕ с РАЗНОЙ величиной (соло,
     ...
     одна величина» стало свойством конструкции, а не соглашением. Прицепка аккордов (revCh) пока
     осталась фиксированной и вне цепи — её переносит Пласт 3.5.2.
...
function makeReverbFx(){
  const inNode=AC.createGain();  inNode.gain.value=1;     // ← у соло этот узел держит переменная verb
  const outNode=AC.createGain(); outNode.gain.value=0.9;  // ← у соло этот узел держит переменная verbOut
...
async function initAudio(){
  ...
  /* Общий ревербератор: СЕТЬ ЗАДЕРЖЕК С ОБРАТНОЙ СВЯЗЬЮ (FDN, см. buildFDN), у каждого источника свой
     ...
     ⚠️ verb остался ВХОДНЫМ УЗЛОМ и сохранил ИМЯ намеренно: оба посыла — соло (leadOut→send→verb, где
     send принадлежит экземпляру) и аккорды (chordBus→revCh→verb) — приходят сюда. АККОРДОВЫЙ ПОСЫЛ В
     3.5.1 НЕ ТРОНУТ: аккорды по-прежнему звучат в комнате СОЛО, своей цепи у них ещё нет (это 3.5.2).
  ...
  /* ЭКЗЕМПЛЯР РЕВЕРБА РОЛИ СОЛО. Строим ЯВНО И ЗДЕСЬ, а не лениво, по двум причинам: реверб стоит в
     ...
     прежним — сеть до соло-цепочки, verb/verbOut существуют к моменту, когда ниже цепляются посылы.
     Прочие роли строятся ЛЕНИВО (fxInstance), поэтому пустая цепь баса/ударных не стоит ничего. */
  const revLd=fxInstance('ld','reverb');
  verb=revLd.in; verbOut=revLd.out;   // ⚠️ СТАРЫЕ ИМЕНА = ТЕ ЖЕ УЗЛЫ: строка посыла АККОРДОВ (revCh) ниже не тронута
