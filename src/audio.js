...
const FX_FACTORY={
  ...
  dly:   { id:'dly',    labelKey:'fx.dly',    kind:'bus',   params:DLY_PARAMS,    make:makeDelayFx },   // в.1: fxId оставлен прежним 'dly' — это СТАБИЛЬНЫЙ идентификатор (цепь, метаданные показа, словарь), переименовывать его ради красоты незачем
};
...
function fxAttach(role,inst){
  if(inst && inst.kind==='voice') return;   // ⛔ У ГОЛОСОВОГО ЭФФЕКТА УЗЛОВ НЕТ: bus.connect(inst.send) с undefined упал бы ТУТ ЖЕ, на первом обращении к цепи. Цеплять нечего — величину читает сам голос на атаке
  const bus=FX_ROLE_BUS[role];
  if(!bus||!inst||inst.wired) return;
...
function fxRoleBus(role,node){
  FX_ROLE_BUS[role]=node;
  const byRole=FX_INST[role]; if(!byRole) return;
  for(const id in byRole) fxAttach(role,byRole[id]);
}
