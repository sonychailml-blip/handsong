...
let banks=[], bldHum=null, bldVib=null, vibLFO, tremLFO, dlyBus, revBus,
...
/* ⚠️ vibGain / tremGain / tremDepth / dlyWet УДАЛЕНЫ (Пласт 3.7.1). Это были ОБЩИЕ узлы глубины на всю
   ...
   остались только ГЕНЕРАТОРЫ (vibLFO/tremLFO) и ЛИНИЯ ЗАДЕРЖКИ (с в.1 — внутри экземпляра делея соло, dlyBus — его затвор) — их делить и надо:
...
/* ЭКЗЕМПЛЯР РЕВЕРБА. Строится ПО ОДНОМУ НА РОЛЬ (см. fxInstance ниже) — оттого и «фабрика», а не
   ...
   ⚠️ ЭКЗЕМПЛЯР ВЛАДЕЕТ СВОИМ ПОСЫЛОМ (send) — ключевая правка Пласта 3.5.1. Прежде параметр «подмес»
   писал МОДУЛЬНУЮ переменную revLead, то есть ЖЁСТКО соло-посыл: второй экземпляр физически не смог
   бы иметь свой подмес. Теперь узел посыла принадлежит экземпляру, а прицепка к шине — дело
   вызывающего: roleBus.connect(inst.send). Это ровно находка Пласта 1 «ПОДМЕС — свойство ПРИЦЕПКИ»,
   доведённая до конца: у каждой прицепки свой узел, и потому своя величина.
   ⚠️ send→in соединяем ЗДЕСЬ, внутри фабрики: снаружи посыл невозможно прицепить неверно. */
...
/* ═══ ДЕЛЕЙ — ШИННЫЙ ЭФФЕКТ ЦЕПИ (слайс в.1) ═══
   ...
   ⚠️ ДВЕ ПРИЦЕПКИ, КАК У РЕВЕРБА:
     ...
     голос соло → gate  — пер-голосовой dlySend (Пласт 3.7.1), МИМО send: величина ПЕР-НОТНАЯ, едет в
                          ...
                          делеем. Положи её в send — она умножилась бы на величину шины.
...
const DLY_PARAMS=[
  {key:'mix', labelKey:'fx.dly.mix', short:'DLY', unit:'', def:0.25,       min:0,    max:1,             curve:'lin', attach:true},
...
const DLY_MIX=DLY_PARAMS[0];   // спецификация подмеса (как REV_MIX): по ней нормированное значение события превращается в величину посыла
function makeDelayFx(){
  const send=AC.createGain(); send.gain.value=0;   // ПОСЫЛ ШИНЫ роли; стартовый 0, дефолт вносит mix.setNorm ниже (как у реверба)
  const gate=AC.createGain(); gate.gain.value=1;   // ВКЛЮЧЕНИЕ эффекта в цепи роли (пишет только fxSetActive); сюда же цепляются голоса соло
  const line=AC.createDelay(DLY_A.maxTime); line.delayTime.value=DLY_A.time;
  const fb=AC.createGain(); fb.gain.value=DLY_A.fb;
  const out=AC.createGain(); out.gain.value=1;     // 1, а не 0.9 как у реверба: прежняя линия шла в мастер НАПРЯМУЮ (dly.connect(master)), единица делает замену бит-в-бит
  send.connect(gate); gate.connect(line); line.connect(fb); fb.connect(line); line.connect(out);
  const SET={
  ...
  return { id:'dly', labelKey:'fx.dly', kind:'bus', in:line, out, send, gate, params };
}
/* ═══ ТРЕМОЛО (МИКС) — ШИННАЯ ВСТАВКА ЦЕПИ (слайс в.2) ═══
   ...
   ⚠️ ПОЧЕМУ ВСТАВКА, А НЕ ПОСЫЛ — и почему прицепке понадобился третий род (см. fxAttach). Тремоло
...
/* ═══ ЭФФЕКТ-ПО-ГОЛОСАМ: ВТОРОЙ ВИД КРЕПЛЕНИЯ (цель «а») ═══
   ...
   {get,set}. Узлы нужны РОВНО ДВУМ местам — fxAttach (прицепить к шине) и fxSetActive (гасить gate).
...
/* ═══ ЛЕНИВАЯ ПРИЦЕПКА ЦЕПИ К ШИНЕ РОЛИ (Пласты 3.5.3/3.5.4: бас и ударные) ═══
   ...
   ⛔ СОЛО ЗДЕСЬ НЕ РЕГИСТРИРУЕТСЯ, И ЭТО НЕ ЗАБЫВЧИВОСТЬ: его прицепка ПЕР-ГОЛОСОВАЯ — голоса цепляются в
   gate экземпляра, минуя send (revBus/dlyBus в initAudio), шины для посыла у соло нет вовсе. Зарегистрируете
   соло — его leadOut польётся в send ВДОБАВОК к пер-нотным посылам, и роль поедет с удвоенным эффектом.
   ⚠️ АККОРДЫ РЕГИСТРИРУЮТСЯ С в.1. Прежде их реверб цеплялся ЯВНОЙ строкой в initAudio, и делею (любому
   новому шинному эффекту) прицепиться к аккордам было бы нечем. Явная строка СНЯТА вместе с регистрацией:
   оставь её — реверб аккордов прицепится ДВАЖДЫ. Флаг inst.wired страхует только путь через fxAttach. */
/* ═══ ТРИ РОДА ПРИЦЕПКИ (третий — слайс в.2) ═══
     'voice'  — узлов нет, величину читает голос на атаке (яркость). Цеплять нечего.
     'bus'    — ПОСЫЛ: шина → send → сеть → мастер, ПАРАЛЛЕЛЬНО сухому ходу (реверб, делей).
...
async function initAudio(){
  ...
  /* ⚠️ С в.1 ЛИНИЯ ПРИНАДЛЕЖИТ ЭКЗЕМПЛЯРУ ДЕЛЕЯ СОЛО (makeDelayFx), а не initAudio — точно как комната
     соло (revLd выше). Константы те же (0.35 с, повторы 0.45), выход в мастер тот же, а между посылами и
     линией встал затвор с усилением 1 — замена бит-в-бит. Голоса соло цепляются в ЗАТВОР (dlyBus) мимо
  ...
  /* ⚠️ СОЛО БОЛЬШЕ НЕ ШЛЁТ В РЕВЕРБ С ШИНЫ (Пласт 3.7.2). Прежде здесь стояло leadOut.connect(revLd.send):
     ...
     через revChI.send: у них один источник, пер-нотного подмеса нет, и значение цепи у аккордов ЖИВОЕ (см.
     шапку makeDelayFx). Пласт 3.8, обещавший это разрешить, ⛔ ОТМЕНЁН; разрешит ЗАМОРОЗКА дорожки — она ещё впереди. */
  revBus=revLd.gate;
  /* ПРОСТРАНСТВО (наклон ладони → ЭХО): ОТДЕЛЬНАЯ линия ДЕЛЕЯ руки-выразительности — НЕ реверб, чтобы читалось
     ...
     делею fx-руки (линия в экземпляре делея соло + пер-голосовые посылы), чьи значения она НЕ читает и НЕ пишет. Нейтраль: посыл 0. */
  ...
  chordBus=AC.createGain(); chordBus.gain.value=0.28; chordBus.connect(master);   // СУХОЙ ход — не тронут
...
function applyFx(m){
  ...
  /* ДЕЛЕЙ — МОДУЛЬ с в.1: ключ 'dly:mix' (прежде 'dly:amt' старого скалярного, по форме как 'reverb:mix').
     Значение — то же нормированное число 0..1, что лежало в state.fx.dly: у mix min 0 / max 1, и fxDenorm —
     точное тождество. Именно поэтому pendFx.dly остаётся НОРМИРОВАННЫМ, а масштаб посыла (DLY_A.send)
     применяет голос — в отличие от pendFx.rev, где в карте лежит нормированное, а сюда кладётся сырое. */
  const dm=s['dly:mix'];
  pendFx.dly = dm==null ? 0 : fxDenorm(DLY_MIX,dm);
...
function newLeadVoice(){
  ...
  const dlySend=AC.createGain(); dlySend.gain.value=pendFx.dly*DLY_A.send*LEAD_OUT_G;   // ПОСЫЛ этого голоса в ЗАТВОР делея соло (линия — в экземпляре, в.1); DLY_A.send — тот же 0.55, ЕДИНЫЙ источник с сеттером шины
  ...
  trem.connect(dlySend); dlySend.connect(dlyBus);
...
function applyVoiceFx(v,p,t,fresh){
  ...
  put(v.dlySend.gain, p.dly*DLY_A.send*LEAD_OUT_G, 0.08);
...
export {
  ...
  FX_FACTORY, fxInstance, fxSetActive, fxSnapshot, fxChordBri,   // ЧТО существует (статика, есть до initAudio — по ней меню строит список), ЧТО построено (живой экземпляр роли: gestures пишет параметры, draw читает величины для столбиков) и ВКЛ/ВЫКЛ прицепки (ui — при снятии/возврате эффекта в цепь)
};
