...
/* ================= МОДУЛЬ ЭФФЕКТА — ПЛАСТ 1: абстракция + ТОЛЬКО реверб =================
   ...
   • ПОДМЕС (mix) — свойство ПРИЦЕПКИ, а не эффекта: у реверба прицепок ДВЕ с РАЗНОЙ величиной (соло
     revLead, аккорды revCh=0.12), и одним «общим wet на выходе» их не выразить — он двинул бы и хвост
     аккордов. ⚠️ В Пласте 3.1 подмес ВСЁ ЖЕ СТАЛ параметром (третьим), но это НЕ отмена наблюдения, а
     его следствие: параметр пишет ИМЕННО СОЛО-ПОСЫЛ (revLead), а не выход модуля. Прицепка аккордов
     осталась фиксированной и к пальцу отношения не имеет. Разблокировало это ровно то, что играющая
     рука перестала писать revLead каждый кадр (реверб ушёл с неё, см. applyFx).
...
/* РЕЕСТР МОДУЛЕЙ ЭФФЕКТОВ (id → модуль). Пласт 2.3: у абстракции появился ПЕРВЫЙ ПОТРЕБИТЕЛЬ —
   ЦЕПЬ ЭФФЕКТОВ РОЛИ (state.fxChains + меню в ui; до Пласта 3.3 это звалось «раскладкой fx-руки»,
   fxLayout), — поэтому реестр ЭКСПОРТИРУЕТСЯ. В Пласте 1 экспорта не было намеренно: потребителя не
   существовало, и это был бы мёртвый API.
   Заполняется в initAudio: узлы модуля существуют только после него. До старта реестр ПУСТ, и это
   нормально — цепь по умолчанию состоит из четырёх СТАРЫХ скалярных эффектов, модулей ей не надо.
   ⚠️ ЭКЗЕМПЛЯР ЗДЕСЬ ОДИН НА ID, а цепь у каждой роли своя: две роли, назвавшие один эффект, получили
   бы ОДИН узел. Пока не кусается — цепь есть только у соло, и меню не даёт собрать чужую (см.
   renderFxCtl); решать это Пласту 3.5, когда обработка появится на прочих шинах. */
const FX_MODULES={};
...
function makeReverbFx(){
  ...
  /* min/max — диапазон РУЧКИ (власть у клампа сеттера, см. fxWrapParam). short — КОРОТКИЙ межъязыковой
     ...
     ⚠️ ПАРАМЕТРОВ ТРИ, и третий — ПОДМЕС (Пласт 3.1). Открытие Пласта 1 в силе: подмес — свойство
     ПРИЦЕПКИ, а не эффекта, потому что посылов ДВА с РАЗНОЙ величиной (соло revLead, аккорды
     revCh=0.12). Разрешение — не «общий wet на выходе» (он двинул бы и хвост аккордов), а параметр,
     который пишет ИМЕННО СОЛО-ПОСЫЛ. Разблокировалось ровно тем, что играющая рука перестала писать
     revLead каждый кадр (см. applyFx: строки реверба там больше нет).
     ⚠️ max 0.85 · curve 'lin' · постоянная 0.08 — НЕ произвол: они дословно воспроизводят прежнее
     отображение `p.rev*0.85` с той же плавностью, поэтому mix=1 звучит как прежняя полная глубина.
     ⚠️ set НЕ зовётся при постройке: revLead создаётся ПОЗЖЕ makeReverbFx (там же, где посылы), и
     ранний вызов упал бы. Замыкание держит СВЯЗКУ, а не значение, поэтому к первому движению руки
     узел уже на месте; guard — на всякий. Начальное значение вносится в узел явно, рядом с посылом. */
  const params=[
    ...
    {key:'mix',  labelKey:'fx.reverb.mix',  short:'MIX', unit:'',  def:0.25,       min:0,  max:0.85,  curve:'lin',
     set:v=>{ if(revLead) revLead.gain.setTargetAtTime(v, AC.currentTime, 0.08); }},   // ЕДИНСТВЕННЫЙ писатель revLead.gain после Пласта 3.1
  ];
...
async function initAudio(){
  ...
  /* ПОДМЕС СОЛО В РЕВЕРБ — теперь параметр МОДУЛЯ (fx-рука), а не глубина играющей руки (Пласт 3.1).
     Вносим стартовое значение модуля в узел ЗДЕСЬ: сам модуль построен раньше (узла ещё не было), и без
     этой строки p.cur и revLead.gain разошлись бы — комната показывалась бы включённой, а звучала бы
     сухо. Дефолт НЕнулевой намеренно: реверб не назначен ни на один палец «из коробки» был бы
     недостижим, и приложение молча потеряло бы свою узнаваемую комнату. */
  { const mp=FX_REV&&FX_REV.params.find(p=>p.key==='mix'); if(mp) mp.setNorm(mp.cur); }
