...
/* ═══ ЭФФЕКТ-ПО-ГОЛОСАМ: ВТОРОЙ ВИД КРЕПЛЕНИЯ (цель «а») ═══
   ...
   ⛳ ЭТО ПРЕЦЕДЕНТ ДЛЯ ЦЕЛИ (в): драйв и вибрато на аккордах/басу — ТОТ ЖЕ РОД. У драйва шейпер стоит в
   голосе до огибающей, у вибрато LFO идёт в detune осцилляторов голоса; ни тот, ни другой на шину не
   выносится (и не должен). Сделав здесь kind:'voice' правильно, (в) сводится к записи в фабрике плюс
   чтению величины в нужном пуле — без нового архитектурного захода.
...
/* ═══ ЛЕНИВАЯ ПРИЦЕПКА ЦЕПИ К ШИНЕ РОЛИ (Пласты 3.5.3/3.5.4: бас и ударные) ═══
   ...
   ⛔ СОЛО И АККОРДЫ ЗДЕСЬ НЕ РЕГИСТРИРУЮТСЯ, И ЭТО НЕ ЗАБЫВЧИВОСТЬ: их посылы прицеплены ЯВНЫМИ
   строками в initAudio (у соло — к leadOut, который рождается позже реверба; у аккордов — рядом с
   chordBus). Зарегистрируете их здесь, не сняв те строки, — посыл прицепится ДВАЖДЫ, и роль поедет с
   удвоенным ревербом. Флаг inst.wired ниже страхует только путь через fxAttach, но не явные строки. */
const FX_ROLE_BUS={};
...
async function initAudio(){
  ...
  chordBus.connect(revChI.send); revChI.out.connect(master);
