...
/* ⚠️ fxN ЧИТАЕТ ЦЕПЬ СОЛО ВСЕГДА — включая случай, когда bandR зовут с 'ch'/'bs'. Это НАМЕРЕННОЕ
   ...
   ⛔ НЕ «улучшать» это в «цепь рисуемой роли». ⚠️ ПРЕЖНИЙ ДОВОД ЗДЕСЬ УСТАРЕЛ, А ЗАПРЕТ — НЕТ.
   Говорилось: «у аккордов и баса цепи ПУСТЫ, fxBandR(0) вернул бы 0, и легенда уехала бы влево».
   Пустыми они быть перестали — аккордам цепь построил Пласт 3.5.2 (реверб), а цель «а» добавила
   яркость. Но запрет держится по ДРУГОЙ причине, и она сильнее: ЛЮБАЯ смена этого счёта сдвигает
   легенду прочих ролей С ТОГО МЕСТА, ГДЕ ОНА СТОИТ СЕГОДНЯ, — влево при короткой цепи, вправо при
   длинной, и вдобавок делает положение легенды зависимым от ДЛИНЫ ЦЕПИ (ровно та пляска, которую
   fxBarsMaxN выше гасит счётом по максимуму). Резерв у прочих ролей обязан остаться прежним —
   байт-в-байт. Столбики нерасчётным ролям (слайс б.2) поэтому пойдут К ПРАВОЙ КРОМКЕ роли, мимо
   этой арифметики, — как уже стоит индикатор ЯРК у аккордов. */
...
function drawPhone(res){
  ...
  if(splitOn){
    ...
    const solo=halves.find(h=>h.role==='ld'); if(solo&&roleHasFx('ld'))drawFxBars('ld',solo.rx0,H);   // столбики у соло-половины — ТОЛЬКО при руке-эффектах: с уходом REV (Пласт 3.1) без неё рисовать нечего
    ...
    if(phoneInstr==='ld'&&roleHasFx('ld'))drawFxBars('ld',0,H);   // ТОЛЬКО при руке-эффектах: столбик REV (он жил по roleHasNotes) удалён в Пласте 3.1 — реверб теперь обычный слот раскладки
...
function drawFxBars(role,rx0,H){                   // rx0 — левый край роли (столбики от него); прежний 1-й арг W не использовался. РОЛЬ — с б.1; оба вызова ниже пока передают 'ld' (новых столбиков слайс не даёт, это б.2)
...
/* ⚠️ ПОЧЕМУ ЭТОТ СТОЛБИК НЕ ХОДИТ ЧЕРЕЗ fxBarItems/drawFxBars, хотя яркость теперь обычный эффект цепи.
   Общий путь СТРОГО СОЛО-ОБРАЗНЫЙ по трём независимым признакам: он выкладывает столбики СТОПКОЙ ОТ
   ЛЕВОГО КРАЯ (у аккордов слева палитра — там рисовать нельзя, потому этот и стоит у ПРАВОГО), он
   гейтится `roleHasFx('ld')` (у аккордов fx-руки не бывает по устройству), и его счёт кормит АРИФМЕТИКУ
   ОТСТУПА под легенду (fxN/bandR), где чужая роль СОЗНАТЕЛЬНО берёт соло-число (см. R-1). Завести туда
   аккордовый столбик — значит параметризовать обход ролью, переучить раскладку на правый край и тронуть
   тот самый счёт отступа, который специально оставлен межролевым. Цена не окупается ради одного
   столбика: вместо этого он подчиняется тому же ЗАКОНУ (есть в цепи — рисуется), оставаясь при своей
   геометрии. Придут другие голосовые эффекты со своими столбиками — вот тогда общий путь и обобщать.
