...
/* ГЕОМЕТРИЯ СТОЛБИКОВ — ОТ ПРАВОЙ КРОМКИ РОЛИ (слайс б.2; прежде FX_X0=6 и стопка от ЛЕВОЙ).
   ...
   FX_EDGE — КРАЙНЕЕ ГНЕЗДО, и оно ЗАНЯТО НЕ НАМИ: там живут одиночные индикаторы роли — «ВЫР»
   (drawExprBar, соло) и «ЯРК» (drawChordBright, аккорды), оба по формуле rx1-FX_BAR_W-10. Цепочка
   эффектов начинается СЛЕДУЮЩИМ гнездом, поэтому у соло с рукой-выразительностью И рукой-эффектами
   ничего не наезжает друг на друга: ВЫР у кромки, эффекты левее с обычным зазором.
...
/* «ЭФФЕКТ ЕСТЬ В ЦЕПИ ЭТОЙ РОЛИ» — тот же вопрос, на который отвечает обход цепи в fxBarItems, только
   для эффектов, у которых столбик рисуется ОТДЕЛЬНО (сегодня один такой — яркость аккордов). Держим
   рядом с прочими читателями цепи: показ обязан следовать за составом цепи, а не за ролью. */
const fxInChain=(role,id)=> fxChainOf(role).some(e=>e&&e.fxId===id);
...
function drawRole(instr,rx0,rx1,playH){
  ...
  /* Индикатор Z-яркости — пока звучит защёлкнутый аккорд (та же связка, что разбор Гц выше) И пока
     ЯРКОСТЬ ЕСТЬ В ЦЕПИ РОЛИ. ⚠️ Последнее условие добавлено вместе со слайсом «а»: столбик старше
     модели цепей и рисовался ПО РОЛИ, безусловно, — поэтому снятая из цепи яркость переставала звучать,
     а полоса оставалась висеть (и, что хуже, показывала нейтраль как полную). Теперь яркость —
     обычный эффект цепи, и её показ обязан подчиняться тому же закону, что у прочих: нет в цепи —
     нет столбика (у остальных это выходит само собой, они строятся ОБХОДОМ цепи в fxBarItems). */
  if(instr==='ch'&&latchDeg>=0&&supportsChords()&&fxInChain('ch','bright'))drawChordBright(rx0,rx1,playH);
}
function drawPhone(res){
  ...
  if(splitOn){
    ...
    for(const h of halves) drawFxBars(h.role,h.rx1,H);   // столбики КАЖДОЙ половины — у ЕЁ правой кромки (слайс б.2; прежде только соло-половина и от левой). Гейт roleHasFx живёт ВНУТРИ: у половины без руки-эффектов список пуст и не рисуется ничего
    ...
    drawFxBars(phoneInstr,W,H);   // столбики текущей роли у правой кромки экрана (слайс б.2). Гейт roleHasFx внутри — роль без руки-эффектов рисует пустой список
...
function drawFxBars(role,rx1,H){                   // rx1 — ПРАВАЯ кромка роли (слайс б.2; было rx0 — левая). Зовётся для ЛЮБОЙ роли: пустой items = ничего не рисуем
  const items = roleHasFx(role) ? fxBarItems(role) : [];
  const y1=H-40, y0=y1-FX_BAR_MAX;                // низ справа: выше строки статуса, ниже коробки лупера (та вверху по центру)
...
/* ⚠️ ТРИ ДОВОДА, ПОЧЕМУ ЭТОТ СТОЛБИК НЕ ХОДИЛ ЧЕРЕЗ fxBarItems/drawFxBars, — ВСЕ ТРИ УМЕРЛИ.
   Здесь стояло: общий путь (1) выкладывает столбики СТОПКОЙ ОТ ЛЕВОГО КРАЯ, (2) гейтится
   `roleHasFx('ld')`, (3) его счёт кормит арифметику отступа под легенду (fxN/bandR, правило R-1).
   Слайс б.1 снял (2) — гейт стал `roleHasFx(role)`. Слайс б.2 снял (1) и (3) разом: столбики ушли к
   ПРАВОЙ кромке у всех ролей, а левый резерв (вместе с R-1) перестал существовать.
   ⚠️ ЗНАЧИТ, ЕДИНСТВЕННАЯ ПРИЧИНА, ПО КОТОРОЙ ЯРК ЕЩЁ ОТДЕЛЬНЫЙ, — ЧТО У АККОРДОВ ПОКА НЕТ РУКИ-
   ЭФФЕКТОВ (в HANDFN_OPTS.ch нет 'fx' — это слайс б.3). Пока её нет, drawFxBars('ch',…) рисует пустой
   список, наложения нет, и ЯРК спокойно занимает КРАЙНЕЕ ГНЕЗДО (FX_EDGE) — то самое, что у соло
   занимает «ВЫР».
   ⛔ СЛАЙС б.3 ОБЯЗАН ЭТО РАЗРЕШИТЬ, И ЭТО НЕ КОСМЕТИКА: как только у аккордов появится fx-рука,
   яркость станет рисоваться ДВАЖДЫ — здесь и обычным столбиком цепи, — причём ВЗАИМНО ИНВЕРТИРОВАННО
   (здесь показывается chBrightDisp = 1-bri, а столбик цепи показал бы сам параметр). Решение записано
   в плане цели (б): при наличии fx-руки у роли яркость рисует ОБЩИЙ путь, а этот столбик гаснет.
...
function drawChordBright(rx0,rx1,H){
  ...
  const x=rx1-FX_EDGE, y1=H-40, y0=y1-FX_BAR_MAX;   // КРАЙНЕЕ гнездо. Число ТО ЖЕ (FX_EDGE=FX_BAR_W+10 — прежняя формула буква в букву), но выражено общей константой: цепочка эффектов отсчитывается от неё же, и разъехаться им теперь нечем
  ctx.textAlign='center'; ctx.textBaseline='alphabetic'; ctx.font='11px system-ui';
  ctx.fillStyle='rgba(255,255,255,.09)'; ctx.fillRect(x,y0,FX_BAR_W,FX_BAR_MAX);      // трек
